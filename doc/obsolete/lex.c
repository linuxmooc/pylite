...
{
    fprintf(stderr, "%s:%d: %s\n", lex->file_name, lex->line_number, msg);
    exit(EXIT_FAILURE);
}
...
keyword_t *lex_lookup_keyword(lex_t *this, char *name)
{
    int i;
...
ast_t *lex_new_token(lex_t *this, int token)
{
    ast_t *ast = ast_new_token(token);
    ast->file_name = this->file_name;
    ast->line_number = this->line_number;
    return ast;
}

ast_t *lex_new_keyword(lex_t *this, keyword_t *keyword)
{
    ast_t *ast = lex_new_token(this, keyword->value);
    ast->svalue = keyword->name;
    return ast;
}
...
ast_t *lex_get_word(lex_t *this)
{
    ...
    keyword_t *keyword = lex_lookup_keyword(this, text.data);
    if (keyword) {
        text_destroy(&text);
        return lex_new_keyword(this, keyword);
    }

    ast_t *token = lex_new_token(this, TOKEN_ID);
...
ast_t *lex_get_double(lex_t *this)
{
    ...
    ast_t *token;
    token = lex_new_token(this, TOKEN_DOUBLE_CONST);
...
ast_t *lex_get_string(lex_t *this)
{
    ...
    ast_t *token = lex_new_token(this, TOKEN_STRING_CONST);
    token->svalue = strdup(text.data);
    text_destroy(&text);
    return token;
}
...
ast_t *lex_get_operator(lex_t *this)
{
    ...
    keyword_t *keyword;
    while (1) {
        keyword = lex_lookup_keyword(this, text.data);
        if (keyword)
            return lex_new_keyword(this, keyword);
...
void lex_read_tabs(lex_t *this)
{
    ...
    if (this_tabs > last_tabs) {
        ...
        token = lex_new_token(this, TOKEN_PUSH);
    } else {
        count = last_tabs - this_tabs;
        token = lex_new_token(this, TOKEN_POP);
    }
...
ast_t *lex_meet_eof(lex_t *this)
{
    for (int i = 0; i < this->last_tabs; i++) {
        ast_t *token = lex_new_token(this, TOKEN_PUSH);
        stack_push(&this->token_stack, token);
    }
    this->last_tabs = 0;
